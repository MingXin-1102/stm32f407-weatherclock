...
static void time_sync(void)
{
    ...
    {
        log_printf("[SNTP] get time failed\n");
    ...
    {
        log_printf("[SNTP] invalid date formate\n");
        restart_sync_delay = SECONDS(1);
        goto err;
    }

    log_printf("[SNTP] sync time: %04u-%02u-%02u %02u:%02u:%02u (%d)\n",
...
static void wifi_update(void)
{
    ...
    {
        log_printf("[AT] wifi info failed\n");
        return;
    }
    ...
    if (info.connected)
    {
        log_printf("[WIFI] connected to %s\n", info.ssid);
    ...
    {
        log_printf("[WIFI] disconnected from %s\n", last_info.ssid);
...
static void inner_update(void)
{
    ...
    {
        log_printf("[AHT20] start measure failed\n");
        return;
    }

    if (!aht20_wait_for_measure())
    {
        log_printf("[AHT20] wait for measurefailed\n");
        return;
    }
    ...
    {
        log_printf("[AHT20] read measure failed\n");
        return;
    }
    ...
    log_printf("[AHT20] Temperature: %.1f, Humidity: %.1f\n", temperature, humidity);
...
static void outdoor_update(void)
{
    ...
    const char *weather_url = "https://api.seniverse.com/v3/weather/now.json?key=SACM4ELUFLi5atgdN&location=Nanjing&language=en&unit=c";
    const char *weather_http_response = esp_at_http_get(weather_url);
    if (weather_http_response == NULL)
    {
        log_printf("[WEATHER] http error\n");
        return;
    }

    if (!parse_seniverse_response(weather_http_response, &weather))
    {
        log_printf("[WEATHER] parse failed\n");
        return;
    }
    ...
    log_printf("[WEATHER] %s, %s, %.1f\n", weather.city, weather.weather, weather.temperature);

    main_page_redraw_outdoor_temperature(&weather);
